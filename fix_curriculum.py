...
# 1. First, restore the corrupted find_lesson function.
# We know it starts at 'def find_lesson(lid: str) -> Dict[str, Any]:'
pattern = r'def find_lesson\(lid: str\) -> Dict\[str, Any\]:.*'
restored_functions = """def find_lesson(lid: str) -> Dict[str, Any]:
    for l in CURRICULUM:
        if l["id"] == lid:
            return l
    raise KeyError(lid)
"""
# Remove everything from the start of find_lesson to the end
content = re.sub(pattern, restored_functions, content, flags=re.DOTALL)

# 2. Now find the correct end of the CURRICULUM array. 
# It should be the ']' right before 'def all_levels'
pos = content.find("\n]\n\n\ndef all_levels()")
if pos != -1:
    # Insert new lessons before the closing bracket of the array
    insert_pos = pos + 2 # Pointing to the \n before ]
...
"""
    content = content[:insert_pos] + new_lessons + content[insert_pos:]
    with open("pyforge/curriculum.py", "w", encoding="utf-8") as f:
        f.write(content)
    print("Curriculum fixed and updated.")
else:
